...
@implementation Lession4ContentBoard
...
- (void)handleUISignal:(BeeUISignal *)signal
{
	...
	if ( [signal isKindOf:BeeUIBoard.SIGNAL] )
	{
		if ( [signal is:BeeUIBoard.CREATE_VIEWS] )
		{
			...
			_textView.contentInset = UIEdgeInsetsMake( 0, 0, 44.0f + 30.0f, 0.0f );
	...
	else if ( [signal isKindOf:@"my."] )				// 如果是用户BUTTON发过来的
	{
		if ( [signal is:MY_SIGNAL_PUSH] )				// 如果是用户自定义信号
		{
			...
			Lession4ContentBoard * board = [[[Lession4ContentBoard alloc] init] autorelease];
